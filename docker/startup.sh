...
# install workflows if the user want to
if [[ ! -z $GALAXY_AUTO_UPDATE_WORKFLOWS ]]
    then
        /tool_deps/_conda/bin/galaxy-wait -g http://127.0.0.1 -v --timeout 120 > $GALAXY_LOGS_DIR/post-start-actions.log 
        OLDIFS=$IFS
        IFS=','
        for WFL in `echo "$GALAXY_AUTO_UPDATE_WORKFLOWS"`
        do
            echo "Installing workflow from $WFL" >> $GALAXY_LOGS_DIR/post-start-actions.log
            workflow-install -w $WFL -g http://127.0.0.1 -a $GALAXY_DEFAULT_ADMIN_KEY --publish_workflows >> $GALAXY_LOGS_DIR/post-start-actions.log 
        done
        IFS=$OLDIFS
fi
