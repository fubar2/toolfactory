...
class GalaxyXmlParser(object):
    ...
    def _init_tool(self, xml_root):
        ...
        tool = gxt.Tool(
            ...
            workflow_compatible=xml_root.attrib.get(
    ...
    def _load_version_command(self, tool, vers_root):
        ...
        """
        logger.info(
            "<version_command> is loaded during initiation of the object."
        )
    ...
    def _load_stdio(self, tool, stdio_root):
        ...
        """
        logger.info(
            "<stdio> is not loaded but automatically generated by galaxyxml."
        )
    ...
    def _load_requirements(self, tool, requirements_root):
        ...
        for req in requirements_root:
            ...
            if req.tag == "requirement":
                ...
                logger.warning(
                    req.tag + " is not a valid tag for requirements child"
                )
...
class InputsParser(object):
    ...
    def _load_options_select(self, root, options):
        ...
        for opt_child in options:
            try:
                getattr(self, "_load_{}_options".format(opt_child.tag))(
                    opts, opt_child
                )
            except AttributeError:
                logger.warning(
                    opt_child.tag + " tag is not processed for <options>."
                )
        root.append(opts)
    ...
    def _load_select_param(self, root, sel_param):
        ...
        for sel_child in sel_param:
            try:
                ...
                logger.warning(
                    sel_child.tag
                    + " tag is not processed for <param type='select'>."
                )
    ...
    def _load_param(self, root, param_root):
        ...
        try:
            getattr(self, "_load_{}_param".format(param_type))(
                root, param_root
            )
        except AttributeError:
    ...
    def _load_conditional(self, root, conditional_root):
        ...
        """
        value_ref_in_group = conditional_root.attrib.get(
            "value_ref_in_group", None
        )
    ...
    def load_inputs(self, root, inputs_root):
        ...
        for inp_child in inputs_root:
            try:
                getattr(self, "_load_{}".format(inp_child.tag))(
                    root, inp_child
                )
            except AttributeError:
                logger.warning(
...
class OutputsParser(object):
    ...
    def _load_data(self, outputs_root, data_root):
        ...
        for data_child in data_root:
            try:
                getattr(self, "_load_{}".format(data_child.tag))(
                    data, data_child
                )
            except AttributeError:
                logger.warning(
                    data_child.tag + " tag is not processed for <data>."
                )
    ...
    def _load_collection(self, outputs_root, coll_root):
        ...
        for coll_child in coll_root:
            try:
                getattr(self, "_load_{}".format(coll_child.tag))(
                    collection, coll_child
                )
            except AttributeError:
                logger.warning(
    ...
    def load_outputs(self, root, outputs_root):
        ...
        for out_child in outputs_root:
            try:
                getattr(self, "_load_{}".format(out_child.tag))(
                    root, out_child
                )
            except AttributeError:
                logger.warning(
                    out_child.tag + " tag is not processed for <outputs>."
                )
    ...
    def load_tests(self, root, tests_root):
        ...
        for test_root in tests_root:
            ...
            for test_child in test_root:
                try:
                    getattr(self, "_load_{}".format(test_child.tag))(
                        test, test_child
                    )
                except AttributeError:
                    logger.warning(
